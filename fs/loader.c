...
void* image_load(char *elf_start, unsigned int size/*, char *exec, char *taddr */)
{
    Elf32_Ehdr      *hdr     = NULL;
    ...
    //exec = mmap(size, 1);/*(char*)malloc(size);*///mmap(NULL, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
	exec = (char*)simple_malloc(size);
    ...
    for (i = 0; i < hdr->e_phnum; ++i)
    {
        ...
        {
            monitor_write("image_load:: p_filesz > p_memsz\n");
            //munmap(exec, size);
			//free(exec);
			//munmap(exec);
			simple_free(exec);
    ...
    entry = find_sym("_start", shdr, shdr + symbol_table_index, elf_start, exec);
