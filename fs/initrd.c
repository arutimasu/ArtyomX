...
fs_node_t *initrd_remove(char *name)
{
    ...
    for (i = 0; i < nroot_nodes; i++)
        if (!strcmp(name, root_nodes[i].name))
			...
			nroot_nodes-=1;
			//&root_nodes[i] = root_nodes[--initrd_header->nfiles];
    return 0;
}

/*fs_node_t *initrd_mkdir(char *name)
{
	
	 // Initialise the /dev directory (required!)
    initrd_folder = (fs_node_t*)malloc(sizeof(fs_node_t));
    strcpy(initrd_folder->name, name);
    initrd_folder->mask = initrd_dev->uid =  initrd_folder->gid =  initrd_folder->inode =  initrd_folder->length = 0;
    initrd_folder->flags = FS_DIRECTORY;
    initrd_folder->read = 0;
    initrd_folder->write = 0;
    initrd_folder->open = 0;
    initrd_folder->close = 0;
    initrd_folder->readdir = &initrd_readdir;
    initrd_folder->finddir = &initrd_finddir;
    initrd_folder->ptr = 0;
    initrd_folder->impl = 0;
	
    return initrd_folder;
}*/
